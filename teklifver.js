...
function printProposal() {
    ...
    const annexProject = document.getElementById("annexCentralProject").value;
    const annexPnomatikProject = document.getElementById("annexPnomatikProject").value;
    
    // Create print overlay container dynamically
    const printOverlay = document.createElement("div");
    printOverlay.className = "teklif-print-overlay";
    ...
    proposalItems.forEach((item, idx) => {
        ...
        itemsHTML += `
            <tr>
                ...
                <td style="border: 1px solid #ddd; padding: 6px;">${item.desc}</td>
                <td style="border: 1px solid #ddd; padding: 6px; text-align: center;">${item.qty} ${item.unit}</td>
                <td style="border: 1px solid #ddd; padding: 6px; text-align: right;">$${item.unitPrice.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                <td style="border: 1px solid #ddd; padding: 6px; text-align: right;">$${total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
            </tr>
        `;
    });
    ...
    let annexHTML = "";
    if (annexProject) {
        annexHTML = getCentralAnnexHTML(annexProject);
    }
