...
function initTeklifVer() {
    ...
    // Kalıcı Firma Bilgisi: Ticari Unvan & IBAN (Sistem varsayılanı tanımlıdır, hiçbir kullanıcı elle girmek zorunda kalmaz)
    const defTradeName = localStorage.getItem("t_default_trade_name") || BFT_DEFAULT_TRADE_NAME;
    const defIban = localStorage.getItem("t_default_iban") || BFT_DEFAULT_IBAN;
    const tradeEl = document.getElementById("termsTradeName");
    const ibanEl = document.getElementById("termsIban");
    if (tradeEl && (!tradeEl.value.trim() || tradeEl.value.trim() === "")) {
        tradeEl.value = defTradeName;
    }
    if (ibanEl && (!ibanEl.value.trim() || ibanEl.value.trim() === "") && defIban) {
        ibanEl.value = defIban;
    }
...
function saveCompanyInfoState() {
    ...
    const tradeNameVal = document.getElementById("termsTradeName").value.trim();
    const ibanVal = document.getElementById("termsIban").value.trim();
    if (tradeNameVal) localStorage.setItem("t_default_trade_name", tradeNameVal);
    if (ibanVal) localStorage.setItem("t_default_iban", ibanVal);
...
function buildProposalPrintElement() {
    ...
    const termsTradeName = document.getElementById("termsTradeName").value || localStorage.getItem("t_default_trade_name") || BFT_DEFAULT_TRADE_NAME || "Girilmedi";
    const termsIban = document.getElementById("termsIban").value || localStorage.getItem("t_default_iban") || BFT_DEFAULT_IBAN || "Girilmedi";
...
function loadSelectedProposalTemplate() {
    ...
    try {
        ...
        if (prop.companyInfo) {
            ...
            const defTradeName = localStorage.getItem("t_default_trade_name") || BFT_DEFAULT_TRADE_NAME;
            const defIban = localStorage.getItem("t_default_iban") || BFT_DEFAULT_IBAN;
            document.getElementById("termsTradeName").value = prop.companyInfo.termsTradeName || defTradeName;
            document.getElementById("termsIban").value = prop.companyInfo.termsIban || defIban;
...
function resetToNewProposal() {
    ...
    try {
        ...
        // Firma bilgisi ve standart ticari şartlar varsayılanlarını otomatik koru
        const defTradeName = localStorage.getItem("t_default_trade_name") || BFT_DEFAULT_TRADE_NAME;
        const defIban = localStorage.getItem("t_default_iban") || BFT_DEFAULT_IBAN;
        document.getElementById("termsTradeName").value = defTradeName;
        document.getElementById("termsIban").value = defIban;
        document.getElementById("termsValidity").value = "30.06.2026";
        document.getElementById("termsPayment").value = "%50 PEŞİN KALAN AMBAR TESLİMİ ÖDECEK";
        document.getElementById("termsDelivery").value = "ÖN ÖDEMEDEN SONRA 30 İŞGÜNÜ";
        document.getElementById("termsShipping").value = "ALICIYA AİTTİR";
        document.getElementById("noteBankExchange").value = "TL ÖDEMELERİNDE FİYATLAR FATURA TARİHİNDE KUVEYTTÜRK BANKASI DÖVİZ SATIŞ KURUNDAN TL'YE ÇEVİRİLECEKTİR.";
        document.getElementById("noteOrderConfirm").value = "SİPARİŞİNİZ YAZILI OLARAK TEYİT EDİLDİKTEN VE ÖN ÖDEMEDEN SONRA İŞLEME ALINIR";
